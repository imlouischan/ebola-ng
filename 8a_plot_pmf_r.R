...
Data$vi[20] # not include the estimated in the data
...
# preallocate
pmf_temp = matrix(rep(NA, nrow(samples)*(max(ri)+1)), ncol = max(ri)+1)
colnames(pmf_temp) = 0:max(ri)

# calculation
for (n in 1:nrow(samples)) { # all samples
  R0 = samples$R0[n]
  size = samples$size[n]
  pmf_temp[n, ] = pmf_r(ri = 0:max(ri), R0, size)
}
