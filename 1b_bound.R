...
Data_BH2 = function(i) min(Data$tiD[i], Data_tEND, na.rm = T)
...
Data_BS2 = function(i) {
  BS2 = min(Data$tiH[i], Data$tiD[i], Data_tEND, na.rm = T)
  while(length(i) > 0) { # all descestors
    i = which(Data$vi %in% Data$i[i]) # all infected by i's
    BS2 = min(BS2, Data$tiE[i], Data$tiS[i], Data$tiH[i], Data$tiD[i], na.rm = T)
  }
  return(BS2)
}